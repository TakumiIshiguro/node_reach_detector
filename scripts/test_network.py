...
class deep_learning:
    def __init__(self):
        ...
        # 3camere
        balance_weights = torch.tensor([1.0, 1.5]).to(self.device)
        self.criterion = nn.CrossEntropyLoss(weight=balance_weights)
    ...
    def make_dataset(self, img, intersection_label):
        ...
        t = torch.tensor([intersection_label], dtype=torch.float32,
                         device=self.device)
        print("intersection_label", intersection_label)
    ...
    def save_tensor(self, dataset_tensor, path, file_name):
        os.makedirs(path)
        torch.save(dataset_tensor, path)
        print("save_path: ",path + file_name)
