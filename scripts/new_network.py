...
class Net(nn.Module):
    def __init__(self, n_out):
        ...
        # 特徴抽出器を保持（featuresとavgpoolを使う）
        self.alex_features = alex.features
        self.avgpool = alex.avgpool
        # classifierの一部だけ使う（最初の2層のみ）
        self.alex_fc = nn.Sequential(*list(alex.classifier.children())[:2])  # 4096-dim

        # <LSTM + OUTPUT>
        self.lstm = nn.LSTM(input_size=4096, hidden_size=512, num_layers=2, batch_first=True)
        self.output_layer = nn.Linear(512, n_out)

    def forward(self, x):
        # x: [B, T, C, H, W]
        B, T, C, H, W = x.shape
        frameFeatures = torch.empty(size=(B, T, 4096), device=x.device)

        for t in range(T):
            frame = x[:, t, :, :, :]  # [B, C, H, W]
            f = self.alex_features(frame)        # -> [B, 256, 6, 6]
            f = self.avgpool(f)                 # -> [B, 256, 6, 6]
            f = torch.flatten(f, 1)             # -> [B, 9216]
            f = self.alex_fc(f)                 # -> [B, 4096]
            frameFeatures[:, t, :] = f

        lstm_out, _ = self.lstm(frameFeatures)  # -> [B, T, 512]
...
class deep_learning:
    ...
    def load_dataset(self, image_path, node_path):
        ...
        self.net = Net(n_out=self.num_classes).to(self.device)
    ...
    def make_dataset(self, img, node_num):
        ...
        if self.first_flag:
            ...
            if self.first_time_flag:
                self.img_time = torch.zeros(1, FRAME_SIZE, 3, 227, 227).to(self.device) 
